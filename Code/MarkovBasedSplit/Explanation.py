...
#     saveResults = pd.DataFrame(columns=['Model','MRR','Hit@1','Hit@3','Hit@5','Hit@10'])
    global saveResults
    if entitySubset:
...
#         print("Regular ComplEx- MR: ", mr_score(ranks), "MRR: ", mrr_score(ranks), "Hits@1: ",        hits_at_n_score(ranks, 1), "Hits@3: ", hits_at_n_score(ranks, 3),"Hits@5: ", hits_at_n_score(ranks, 5), "Hits@10: ", hits_at_n_score(ranks, 10)) 

        print("\n Evaluating the FocusE regular")
        ...
        print("CEG Regular - MRR: ", mrr_score(ranks), "MR: ", mr_score(ranks), "Hits@1: ",        hits_at_n_score(ranks, 1), "Hits@3: ", hits_at_n_score(ranks, 3), "Hits@10: ", hits_at_n_score(ranks, 10)) 
        
        print("CEG FocusE - MRR: ", mrr_score(ranksFocusE), "MR: ", mr_score(ranksFocusE), "Hits@1: ",        hits_at_n_score(ranksFocusE, 1), "Hits@3: ", hits_at_n_score(ranksFocusE, 3), "Hits@10: ", hits_at_n_score(ranksFocusE, 10)) 
#         print("FocusE TransE- MR: %f, MRR: %f, Hits@1: %f, Hits@3: %f, Hits@5: %f, Hits@10: %f" % mr_score(ranksFocusE),mrr_score(ranksFocusE), hits_at_n_score(ranksFocusE, 1), hits_at_n_score(ranksFocusE, 3), hits_at_n_score(ranksFocusE, 5), hits_at_n_score(ranksFocusE, 10)) 
    ...
    saveResults = pd.concat([saveResults, pd.DataFrame({'Model':[model_name+'_FocusE_'+folderName], 'MRR':["{:.4f}".format(mrr_score(ranksFocusE))],'MR':["{:.4f}".format(mr_score(ranksFocusE))], 'Hit@1':["{:.4f}".format(hits_at_n_score(ranksFocusE, 1))], 'Hit@3':["{:.4f}".format(hits_at_n_score(ranksFocusE, 3))], 'Hit@10':["{:.4f}".format(hits_at_n_score(ranksFocusE, 10))]})])
...
def generate_focusE_dataset_splits(filterPredicates,folder,split_test_into_top_bottom=True, split_threshold=0.1):
    X_train = pd.read_csv('..Data/CausalCLEVERERHumanKG_MarkovSplitPrediction/Explanation/'+folder+'/train.txt', sep='\t',header=None).to_numpy()
    X_test = pd.read_csv('..Data/CausalCLEVERERHumanKG_MarkovSplitPrediction/Explanation/'+folder+'/test.txt', sep='\t',header=None).to_numpy()
    ...
    dataset = {'train':train, 'test':test, 'train_numeric_values':train_numeric_values, 
...
def callTheSplitFunction(filterPredicates,folderName,model_name, vanilla_params, focusE_params):
    ...
    X_filter = np.concatenate([X['train'], X['test']], 0)
...
#     X_valid = X_valid[(X_valid_numeric_values) >= 0.75] 
    ...
    modelBuilding(X_train,X_test,X_filter,X_train_numeric_values,count, folderName, filterPredicates, model_name, vanilla_params, focusE_params)
    return

def modelBuilding(X_train,X_test,X_filter,X_train_numeric_values,count, folderName, filterPredicates, model_name, vanilla_params, focusE_params):
...
if __name__ == "__main__":    
    json_file_path = './config_Explanation.json'
    ...
    for dataset in datasets:
        ...
        for model_name in model_names:
            ...
            focusE_params = config['regular'][dataset][model_name]
#             compare_vanilla_focusE_models(dataset, model_name, vanilla_params, focusE_params)
            ...
            for i in folderList: 
                if dataset == list(i.values())[0]:
                    ...
                    print("Data Folder: ",list(i.values())[0])
                    callTheSplitFunction(list(i.keys())[0], list(i.values())[0], model_name, vanilla_params, focusE_params)
                    print("\n")
        
saveResults.to_csv('..Data/MarkovBasedSplit/Explantion.csv', index=False)
