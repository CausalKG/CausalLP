...
def generate_focusE_dataset_splits(filterPredicates,folder,split_test_into_top_bottom=True, split_threshold=0.1):
    X_train = pd.read_csv('../Data/CausalCLEVERERHumanKG_MarkovSplitPrediction/Prediction/'+folder+'/train.txt', sep='\t',header=None).to_numpy()
    X_test = pd.read_csv('/../Data/CausalCLEVERERHumanKG_MarkovSplitPrediction/Prediction/'+folder+'/test.txt', sep='\t',header=None).to_numpy()
...
if __name__ == "__main__":    
    json_file_path = './config_Prediction.json'
...
saveResults.to_csv('../Data/MarkovBasedSplit/Prediction.csv', index=False)
